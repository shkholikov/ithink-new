          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-start gap-3">
                    ...
                    {href ? (
                      <a href={href} className="text-sm text-foreground hover:text-[#377dff] transition-colors">{label}</a>
                    ) : (
              ...
              <div className="flex gap-3 mt-6">
                <a href="https://instagram.com/ithinkuz" target="_blank" rel="noopener noreferrer" className="px-4 py-2 text-xs font-medium bg-secondary hover:bg-accent text-muted-foreground hover:text-foreground rounded-lg border border-border transition-colors">
                  Instagram
                </a>
                <a href="https://t.me/ithinkuzbot" target="_blank" rel="noopener noreferrer" className="px-4 py-2 text-xs font-medium bg-secondary hover:bg-accent text-muted-foreground hover:text-foreground rounded-lg border border-border transition-colors">
                  Telegram
                </a>
              </div>
            </motion.div>
            ...
            {/* Form */}
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="bg-card border border-border rounded-2xl p-7">
              {status === 'success' ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <input placeholder={t('form.name')} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required className={inputClass} />
                  ...
                  <textarea placeholder={t('form.message')} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} required rows={4} className={`${inputClass} resize-none`} />
