...
export default function PricingPage({ locale }: { locale: string }) {
  ...
  const plans = [0, 1, 2].map((i) => ({
    ...
    features: [0, 1, 2, 3, 4, 5].map((j) => {
      try { return t(`plans.${i}.features.${j}`); } catch { return null; }
            ...
            {plans.map((plan, i) => (
              <motion.div
                ...
                className={`relative rounded-2xl p-7 flex flex-col ${
                  plan.popular
                    ? 'bg-[#377dff] border border-[#377dff] shadow-lg shadow-[#377dff]/20'
              ...
              >
                ...
                <div className="mb-6">
                  <h3 className={`text-base font-semibold mb-1 ${plan.popular ? 'text-white' : 'text-foreground'}`}>
                    {plan.name}
                  </h3>
                  <p className={`text-xs mb-5 ${plan.popular ? 'text-white/70' : 'text-muted-foreground'}`}>
                    {plan.description}
                  </p>
                  <div className="flex items-baseline gap-1">
                    ...
                    <span className={`text-xs ${plan.popular ? 'text-white/60' : 'text-muted-foreground'}`}>
                      {plan.period}
                    </span>
                  </div>
                </div>
                ...
                <Link
                  ...
                  className={`block text-center py-2.5 text-sm font-medium rounded-xl transition-colors ${
                    plan.popular
                      ? 'bg-white text-[#377dff] hover:bg-white/90'
                      : 'bg-[#377dff] text-white hover:bg-[#2563eb]'
                  }`}
                >
                  {t('getStarted')}
                </Link>
          ...
          >
            ...
            <Accordion className="space-y-3">
              {faqItems.map((item, i) => (
                <AccordionItem
                  key={i}
                  className="bg-card border border-border rounded-xl px-5 data-open:border-[#377dff]/30"
                >
